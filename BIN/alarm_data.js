...
$(document).ready(function() {
    ...
    const tabContainer = $('<div>').addClass('col-md-3').css({
    ...
    const alarmTab = $('<li>').addClass('active');
    const alarmLink = $('<a>').attr('href', '#alarm').attr('data-toggle', 'tab').text('Alarm');
    alarmTab.append(alarmLink);
    
    const eventTab = $('<li>');
    const eventLink = $('<a>').attr('href', '#event').attr('data-toggle', 'tab').text('Event');
    ...
    const alarmContent = $('<div>').addClass('tab-pane active').attr('id', 'alarm');
    const eventContent = $('<div>').addClass('tab-pane').attr('id', 'event');
    ...
    function createSubTabs() {
        ...
        tabs.forEach((tab, index) => {
            ...
            const tabItem = $('<li>').toggleClass('active', isActive);
            const link = $('<a>')
                .attr('href', `#${tab.id}`)
    ...
    $.getJSON('data.json', function(data) {
        function updateContent(tab, subTab, content) {
            ...
            if (subTab !== 'station') {
                // Create the dropdown
                const dropdown = $('<select>').addClass('form-control').css('margin-top', '20px');
                ...
                $('<div>', { class: 'modal-dialog', role: 'document' }).append(
                    $('<div>', { class: 'modal-content' }).append(
                        ...
                        $('<div>', { class: 'modal-footer' }).append(
                            $('<button>', { type: 'button', class: 'btn btn-secondary', 'data-dismiss': 'modal' }).text('Close')
                        )
                    )
                )
            );
            ...
            const tab = navTabs.find('li.active a').attr('href').substring(1);

            // Get the active sub-tab ID based on the active tab
            const subTab = (tab === 'alarm' ? alarmSubTabs.subNavTabs : eventSubTabs.subNavTabs)
                .find('li.active a').attr('href').substring(1);

            // Get the content element corresponding to the active sub-tab
        ...
        function handleTabClick(e) {
            ...
            // Get the active sub-tab id
            const subTab = (tab === 'alarm' ? alarmSubTabs.subNavTabs : eventSubTabs.subNavTabs).find('li.active a').attr('href').substring(1);
            
            // Get the content element
            const content = (tab === 'alarm' ? alarmSubTabs.subTabContent : eventSubTabs.subTabContent).find('#' + subTab);
            
            // Remove active class from all sub-tab contents
            (tab === 'alarm' ? alarmSubTabs.subTabContent : eventSubTabs.subTabContent).find('.tab-pane').removeClass('active');
            
            // Add active class to the current sub-tab content
        ...
        function handleSubTabClick(e) {
            tableContainer.empty()
            const subTab = $(e.target).attr('href').substring(1); // Get the sub-tab id
            const tab = navTabs.find('li.active a').attr('href').substring(1); // Get the active tab id
            const content = (tab === 'alarm' ? alarmSubTabs.subTabContent : eventSubTabs.subTabContent).find('#' + subTab); // Get the content element
        ...
        $(document).on('shown.bs.tab', 'a[data-toggle="tab"][href^="#alarm"], a[data-toggle="tab"][href^="#event"]', handleTabClick);
        
        // Handle sub-tab click events
        $(document).on('shown.bs.tab', 'a[data-toggle="tab"][href^="#station"], a[data-toggle="tab"][href^="#platform"], a[data-toggle="tab"][href^="#door"]', handleSubTabClick);
        
        // Initial content load
        updateContent('alarm', 'station', alarmSubTabs.subTabContent.find('#station'));
    });
});